...
from openrlhf.trainer import SMACTrainer
...
def train(args):
    ...
    if args.aligner_init_on_gpu:
        aligner = aligner.to(torch.cuda.current_device())
    ...
    if args.gradient_checkpointing:
        ...
        aligner.gradient_checkpointing_enable(
            gradient_checkpointing_kwargs={"use_reentrant": args.gradient_checkpointing_use_reentrant}
        )
    ...
    aligner_optim = strategy.create_optimizer(
        aligner, lr=args.actor_learning_rate, betas=args.adam_betas, weight_decay=args.l2
    )
    ...
    aligner_scheduler = get_scheduler(
        "cosine_with_min_lr",
        aligner_optim,
        num_warmup_steps=math.ceil(max_steps * 0.03),
        num_training_steps=max_steps,
    ...
    (
        ...
        (aligner, aligner_optim, aligner_scheduler),
        reward_model,
        initial_model,
        initial_aligner,
    ) = strategy.prepare(
    ...
    if ema_model:
        ...
        ema_aligner._offlid = True
        ema_aligner = strategy.prepare(ema_aligner, is_rlhf=True)

    # load checkpoint
    consumed_samples = 0
    if args.load_checkpoint and os.path.exists(os.path.join(args.ckpt_path, "_actor")):
        _, states = strategy.load_ckpt(actor.model, os.path.join(args.ckpt_path, "_actor"))
        strategy.load_ckpt(aligner.model, os.path.join(args.ckpt_path, "_aligner"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--aligner_init_on_gpu", action="store_true", default=False)
    ...
    parser.add_argument("--pretrain_aligner", type=str, default=None, help="HF model name or path")
